...
def validate_change_decision_for_confirm(
    ...
    review_options = {
        _clean(value).upper()
        for value in decision.get("review_options") or []
        if _clean(value)
    }
    ...
    if review_options and action not in review_options:
        raise ValueError(
    ...
    if action == "ADD_SECONDARY":
        raise ValueError(
            "ADD_SECONDARY is not available until "
            "certificate history storage is enabled."
        )
    ...
    if action not in allowed_confirm_actions:
        raise ValueError(
            "The selected action does not confirm "
            "the certificate: "
            + action
        )
...
def confirm_filing_workflow(
    ...
    if preview.get("hard_blockers"):
        raise ValueError(
            " / ".join(preview["hard_blockers"])
        )

    change_gate = validate_change_decision_for_confirm(
        preview=preview,
        change_action=change_action,
    )

    if preview.get("blockers") and not force:
        raise ValueError(
    ...
    mail_item = (
        (preview.get("request_context") or {})
        .get("matched_mail_item")
        or {}
    )
    ...
    try:
        ...
        if copy_result.status == "DUPLICATE_SKIPPED":
